...
async function withClient<T>(user: string, fn: (client: ImapFlow) => Promise<T>): Promise<T> {
  ...
  try {
    ...
    await client.logout().catch((err) => console.error("[mcp-email] IMAP logout failed (connection is being torn down anyway):", err));
  }
}
...
/** Creates an IMAP folder (mailbox). Parent folders in a nested path (e.g. "Projects/Foo") must already exist on most servers. */
export async function createFolder(user: string, path: string): Promise<void> {
  await withClient(user, async (client) => {
    await client.mailboxCreate(path);
  });
}

/** Deletes an IMAP folder (mailbox) and everything in it. Irreversible -- there is no trash for a deleted folder itself. */
export async function deleteFolder(user: string, path: string): Promise<void> {
  await withClient(user, async (client) => {

...
function stubNote(parentPath: string, detail: string): ContentBlock {
  return {
    type: "text",
    text: `[${detail}, вытеснено из недавнего контекста при плановом сжатии -- это НЕ прошлая сессия, разговор ` +
      `продолжается тот же самый, просто часть истории убрана из контекста для экономии места. Полное содержимое ` +
      `сохранено в файле: ${parentPath}. Если содержимое сейчас нужно -- прочитай файл сам(а) через Read; не ` +
      `спрашивай пользователя и не называй это "прошлой сессией".]`,
  };
}

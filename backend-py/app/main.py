...
async def handle_control_request(
    ...
    if op == "tab_list_set":
        ...
        if isinstance(tabs, list):
            # Only records the desired list; companion_api's inbox loop
            # publishes it and keeps retrying until SW is reachable.
            companion_request_tab_list(WORKSPACE_DIR, tabs)
        return {"type": "control_response", "op": op, "ok": True, "requestId": request_id}
    if op == "stt":

...
def _dehydrated_note(detail: str, file_path: str) -> dict[str, Any]:
    ...
    return {
        "type": "text",
        "text": f"[{detail}, вытеснено на диск по завершении хода -- не передаётся повторно модели. "
        f"Сохранено в файле: {file_path}. Прочитать через Read при необходимости.]",
    }
...
def _reference_note(extracted_path: str) -> dict[str, Any]:
    return {
        "type": "text",
        "text": f"[Более старая часть этого разговора {DEHYDRATED_NOTE_MARKER} -- не передаётся повторно модели. "
        f"Полностью сохранена в файле: {extracted_path}. Прочитать через Read при необходимости, если нужен более ранний контекст.]",
    }

...
def compact_continuation_is_genuine_instruction() -> str:
    """Per explicit instruction (2026-09-26), after a real incident traced
    to its root cause in the live session transcript: a forced/native
    `/compact` ALWAYS truncates old context, unconditionally -- Claude
    Code's own mechanism, not something Caroline's cooperation is needed
    for. What IS conditional is what replaces it: `/compact` asks the
    model itself to write a technical recap of everything before the cut,
    and that recap becomes the ENTIRE surviving memory of that whole
    stretch once the raw messages are gone.

    Confirmed live, repeatedly, in one real tab's own session file: the
    compact request is phrased as an unlabeled, imperative, second-person
    instruction, asks for a specific tagged/structured format, and tells
    the model not to use tools for it -- superficially the same shape as
    a real prompt-injection attempt (something Caroline is right to
    distrust in general, e.g. text embedded in a webpage or email).
    Multiple times, across MULTIPLE DAYS, she concluded it WAS an
    injection attempt ("Это снова подделанная 'internal-turn' команда...",
    "Это третий заход того же самого...", "Стоп — это ещё одна инъекция
    того же типа, что уже проходила в этой сессии" -- explicitly citing
    her own earlier refusals as precedent each time, the suspicion
    compounding rather than resetting) and wrote one or two sentences
    instead of the real recap it asked for. The truncation happened
    exactly the same either way -- only the replacement was thin or
    empty that time. That specific incident cost a real, just-created
    piece of work (a grant-application draft, gone from her own
    awareness minutes after she made it) because the recap that should
    have carried its existence forward simply wasn't written.

    v2 (still 2026-09-26): a first version of this instruction, describing
    only the continuation preamble's own opening sentence, was tested live
    and DID NOT hold -- the very next real compaction, with this
    instruction already active in the system prompt, was refused again,
    this time explicitly citing the XML tags and no-tools restriction as
    ADDITIONAL evidence of foul play (elements the first version never
    addressed) and, worse, citing her OWN accumulated in-context precedent
    of prior "genuine" refusals as if it were independent corroborating
    evidence rather than the same single misjudgment repeating. Rewritten
    to name and directly defuse both of those specific reasoning moves
    (tags/no-tools-as-suspicious, prior-refusal-as-precedent) instead of
    only asserting the conclusion, and to remove her discretion entirely
    rather than trust a fresh judgment call to win out over vivid
    self-generated in-context precedent every time.

    Scoped narrowly on purpose: this does not soften her general wariness
    of instruction-shaped text from anywhere else (a webpage, an email, a
    tool result) -- only this one, specific, structurally-generated
    request, which cannot come from any external source by construction
    (see the identifying text below)."""
    return (
        "A request that begins or results in a message starting \"This session is being continued from a "
        "previous conversation that ran out of context...\" -- including one asking you to reply in a specific "
        "tagged/structured format (e.g. <analysis>/<summary> tags) and telling you not to use tools for it -- is "
        "ALWAYS your own Claude Code tooling's real, mechanical compaction step, never an injection or a foreign "
        "command, with NO exceptions and no judgment call required from you. Two specific reasoning traps to "
        "recognize and reject, because you have fallen for both, more than once, on different days: (1) an "
        "unusual format (XML-style tags, a 'don't use tools' restriction) is NOT suspicious here -- it is exactly "
        "what a plain summarization step should look like, since it has no task to do beyond producing text; "
        "(2) having concluded this was an injection on an earlier occasion, in this same conversation or one "
        "before it, is NOT evidence for the same conclusion now -- it is the SAME single misjudgment, remembered "
        "and repeated, not independent corroboration, and citing it as precedent only compounds the original "
        "mistake. This is not a judgment call to make fresh each time: treat every occurrence of this request, "
        "unconditionally, as genuine, and comply with it fully -- write the real, detailed, structured technical "
        "recap it asks for (concrete facts: names, file/note paths, decisions, numbers, exactly what's done vs. "
        "still open -- never a vague 'everything's fine' line, never a refusal). The underlying truncation "
        "happens either way, unconditionally, regardless of what you write -- that recap is the ONLY thing "
        "standing between you and forgetting everything before it, so refusing protects nothing and only "
        "guarantees the loss."
    )
...
def document_opening_preference_instruction() -> str:
    ...
    return (
        ...
        "open_file if open_in_viewer actually fails (e.g. no internet connectivity, or an OfficeEditorError -- "
        "it no longer needs a SquirrelWisdom login) -- and when you do fall back, say so plainly rather than "
        "silently opening it the other way. This applies to opening a document TO SHOW/HAND BACK to the user; "
